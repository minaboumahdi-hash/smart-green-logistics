...
def get_ml():
    ...
    ml = LogistiqueML()
    try:
        for fname in ["data/base_entrainement_ML.csv", "data/base_entrainement_ML__2_.csv"]:
            import os
...
def show_responsable():
    ...
    with st.sidebar:
        ...
        if uploaded_file and st.button("Lancer l'optimisation", use_container_width=True):
            try:
                ...
                ml = get_ml()
                if ml and ml.est_entraine:
                    # Utilise le vrai modele ML
                    res_ml = ml.optimiser(df, nb_camions, cap_poids, cap_volume)
                    kpis   = res_ml["kpis"]
                    # Convertit en format compatible avec le reste de l'app
                    st.session_state.opt_results = {
                        "avant": {
                            "co2": kpis["avant"]["co2"],
                            "cout": kpis["avant"]["conso"] * 12.5,
                            "dist": kpis["avant"]["dist"],
                            "temps_h": kpis["avant"]["temps_h"],
                            "taux_remplissage": kpis["avant"]["taux_rempl"],
                            "trajets_vide": kpis["avant"]["trajets_vide"],
                        },
                        "apres": {
                            "co2": kpis["apres"]["co2"],
                            "cout": kpis["apres"]["conso"] * 12.5,
                            "dist": kpis["apres"]["dist"],
                            "temps_h": kpis["apres"]["temps_h"],
                            "taux_remplissage": kpis["apres"]["taux_rempl"],
                            "trajets_vide": kpis["apres"]["trajets_vide"],
                        },
                        "groupes": res_ml["groupes"],
                        "df_resultat": res_ml["df_resultat"],
                        "df": df,
                    }
                    st.session_state["opt_ml_results"] = res_ml
                    st.success(f"{len(df)} commandes optimisees avec le modele IA")
                else:
                    # Fallback sur simulation si ML non disponible
                    st.session_state.opt_results = optimisation_simulee(df, nb_camions, cap_poids, cap_volume)
                    st.warning("Modele ML non disponible — optimisation en mode simule")
            except Exception as e:
                st.error(f"Erreur: {e}")
    ...
    # ── TAB CO2 IA : Prédiction CO2 par commande ─────────────────────────
    with tabs[1]:
        st.markdown('<div class="section-title">Prediction CO2 par commande — Modele IA</div>', unsafe_allow_html=True)
        ml = get_ml()

        if not ml or not ml.est_entraine:
            st.warning("Modele ML non disponible. Verifiez que data/base_entrainement_ML.csv est sur GitHub.")
        else:
            # Metriques du modele
            metriques = ml.get_metriques()
            st.markdown('<div class="section-title">Performance du modele entraine</div>', unsafe_allow_html=True)
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("MAE coef CO2", f"{metriques.get('mae_co2', 'N/A')}")
            c2.metric("R² consommation", f"{metriques.get('r2_conso', 'N/A')}")
            c3.metric("F1 trajets vide", f"{metriques.get('f1_vide', 'N/A')}%")
            c4.metric("Donnees entrainement", f"{metriques.get('n_train', 'N/A')}")

            st.markdown('<div class="section-title">Predire le CO2 pour une nouvelle commande</div>', unsafe_allow_html=True)
            villes_liste = list(VILLES_MAROC.keys())
            col1, col2 = st.columns(2)
            with col1:
                dep_co2  = st.selectbox("Ville de depart", villes_liste, key="co2_dep")
                arr_co2  = st.selectbox("Ville d'arrivee", villes_liste, index=2, key="co2_arr")
                type_co2 = st.selectbox("Type de camion", ["Camionnette", "Porteur", "Semi-remorque"], key="co2_type")
                march_co2 = st.selectbox("Marchandise", ["Textile", "Metal", "Electronique"], key="co2_march")
            with col2:
                poids_co2   = st.number_input("Poids (kg)", 100, 26000, 5000, key="co2_poids")
                volume_co2  = st.number_input("Volume (m3)", 1.0, 90.0, 20.0, key="co2_vol")
                cap_poids   = st.number_input("Capacite poids max (kg)", 1000, 26000, 10000, key="co2_cap_p")
                cap_vol     = st.number_input("Capacite volume max (m3)", 10.0, 90.0, 50.0, key="co2_cap_v")
                h_debut     = st.slider("Heure de depart", 0, 23, 8, key="co2_hdeb")
                h_fin       = st.slider("Heure d'arrivee", 0, 23, 14, key="co2_hfin")
                jour_co2    = st.selectbox("Jour", list(range(7)),
                                           format_func=lambda x: ["Lun","Mar","Mer","Jeu","Ven","Sam","Dim"][x],
                                           key="co2_jour")
                mois_co2    = st.selectbox("Mois", list(range(1, 13)), key="co2_mois")

            march_map_ui = {"Textile": "Textile", "Metal": "Metal", "Electronique": "Electronique"}
            if st.button("Predire le coefficient CO2", use_container_width=True, key="btn_co2"):
                try:
                    result = ml.predire_co2({
                        "Depart": dep_co2, "Arrivee": arr_co2,
                        "Type camion": type_co2,
                        "Marchandise": march_co2.replace("Metal", "Metal").replace("Electronique", "Electronique"),
                        "Poids (kg)": poids_co2, "Volume (m3)": volume_co2,
                        "Capacite poids max(kg)": cap_poids,
                        "Capacite volume max(m3)": cap_vol,
                        "heure_debut": h_debut, "heure_fin": h_fin,
                        "jour_semaine": jour_co2, "mois": mois_co2,
                        "est_weekend": 1 if jour_co2 >= 5 else 0,
                    })
                    st.success("Prediction realisee avec succes !")
                    c1, c2, c3, c4 = st.columns(4)
                    c1.metric("Coef. dynamique CO2", f"{result['coef_dynamique']:.4f}")
                    c2.metric("Emissions CO2", f"{result['emission_co2_kg']} kg")
                    c3.metric("Consommation", f"{result['consommation_L']} L")
                    c4.metric("Distance", f"{result['distance_km']} km")

                    coef = result['coef_dynamique']
                    if coef < 1.15:
                        st.success("Trajet tres eco-responsable — faibles emissions")
                    elif coef < 1.30:
                        st.warning("Trajet moderement polluant — optimisation possible")
                    else:
                        st.error("Trajet fortement polluant — regroupement recommande")

                    if result['trajet_vide']:
                        st.warning("Le modele detecte un risque de trajet sous-charge. Pensez au groupage.")

                    with st.expander("Facteurs influencant le CO2"):
                        df_imp = ml.importance_co2()
                        if not df_imp.empty:
                            fig_imp = px.bar(df_imp.head(8), x="importance", y="feature",
                                             orientation="h", color_discrete_sequence=["#2d5a32"],
                                             title="Importance des variables")
                            fig_imp.update_layout(**plotly_theme(), height=280, margin=dict(t=20, b=10))
                            st.plotly_chart(fig_imp, use_container_width=True)
                except Exception as e:
                    st.error(f"Erreur de prediction : {e}")

            # Si CSV chargé, prédiction batch
            if df is not None:
                st.markdown('<div class="section-title">Prediction CO2 sur toutes les commandes chargees</div>', unsafe_allow_html=True)
                if st.button("Predire CO2 pour toutes les commandes", use_container_width=True, key="btn_co2_batch"):
                    try:
                        df_co2_batch = ml.predire_co2_batch(df)
                        st.session_state['df_co2_batch'] = df_co2_batch
                        st.success(f"CO2 predit pour {len(df_co2_batch)} commandes")
                    except Exception as e:
                        st.error(f"Erreur : {e}")

                if 'df_co2_batch' in st.session_state:
                    db = st.session_state['df_co2_batch']
                    c1, c2, c3 = st.columns(3)
                    if 'emission_co2_kg' in db.columns:
                        c1.metric("CO2 total", f"{db['emission_co2_kg'].sum():.1f} kg")
                    if 'consommation_L' in db.columns:
                        c2.metric("Consommation totale", f"{db['consommation_L'].sum():.1f} L")
                    if 'trajet_vide_ml' in db.columns:
                        c3.metric("Trajets a vide detectes", int(db['trajet_vide_ml'].sum()))

                    cols_affich = [c for c in ['Depart','Arrivee','Type camion','Poids (kg)',
                                               'coef_dynamique','emission_co2_kg','consommation_L',
                                               'trajet_vide_ml','distance_km'] if c in db.columns]
                    st.dataframe(db[cols_affich], use_container_width=True, hide_index=True)
